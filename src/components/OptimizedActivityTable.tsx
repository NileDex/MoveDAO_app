    ...
    <div className={`bg-white/3 border border-white/5 rounded-xl p-4 w-full max-w-full overflow-hidden ${className}`} style={{ maxWidth: '100vw' }}>
      ...
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <div className="flex items-center gap-2">
          <h3 className={`text-sm sm:text-base md:text-lg font-semibold flex items-center gap-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>
      ...
      <div className="hidden sm:block overflow-x-auto">
        <table className={`w-full text-sm ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
          <thead>
            <tr className={`border-b ${isDark ? 'border-white/10' : 'border-gray-200'}`}>
              <th className={`text-left py-4 px-4 font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>Activity</th>
              <th className={`text-right py-4 px-4 font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>Type</th>
              {showUserColumn && (
                <th className={`text-right py-4 px-4 font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>User</th>
              )}
              {showAmountColumn && (
                <th className={`text-right py-4 px-4 font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>Amount</th>
              )}
              <th className={`text-right py-4 px-4 font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>Time</th>
              {showDAOColumn && (
                <th className={`text-left py-4 px-4 font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'} hidden md:table-cell`}>DAO</th>
              )}
              {showActionColumn && (
                <th className={`text-right py-4 px-4 font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>Action</th>
              )}
            </tr>
          </thead>
          <tbody>
            {displayActivities.length === 0 ? (
              ...
              displayActivities.map((activity, index) => {
                ...
                return (
                  <tr key={activity.id || index} className={`border-b ${isDark ? 'border-white/5 hover:bg-white/5' : 'border-gray-100 hover:bg-gray-50'} transition-all`}>
                    {/* Activity */}
                    <td className="py-4 px-4">
                      <div className="min-w-0">
                        <h4 className={`font-medium text-sm leading-tight truncate ${isDark ? 'text-white' : 'text-gray-900'}`}>{activity.title}</h4>
                        <p className={`text-xs leading-tight truncate ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{activity.description}</p>
                      </div>
                    </td>
                    ...
                    {showAmountColumn && (
                      <td className="py-4 px-4">
                        <div className="flex justify-end">
                          {activity.amount ? (
                            <span className={`text-sm font-medium ${isDark ? 'text-white' : 'text-black'}`}>{formatAmount(activity.amount)}</span>
                          ) : (
                    ...
                    {/* Time */}
                    <td className="py-4 px-4 text-right">
                      <div className={`flex items-center justify-end space-x-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                    ...
                    {showDAOColumn && (
                      <td className="py-4 px-4 hidden md:table-cell">
                        <span className={`text-sm font-mono ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{truncateAddress(activity.dao)}</span>
                      </td>
                    )}
                      ...
                      <td className="py-4 px-4 text-right">
                        {activity.transactionHash && activity.transactionHash !== '0x' ? (
                          <a
                            ...
                            className={`inline-flex p-2 rounded-lg transition-all ${isDark ? 'text-gray-400 hover:text-white hover:bg-white/5' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'}`}
            ...
            {displayActivities.map((activity, index) => {
              ...
              return (
                <div
                  ...
                  className={`p-2.5 transition-all border-b last:border-b-0 ${isDark ? 'hover:bg-white/5 border-white/5' : 'hover:bg-gray-50 border-gray-100'}`}
                >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center space-x-3 text-xs text-gray-400">
                          ...
                          {showAmountColumn && activity.amount && (
                            <div className={`font-medium ${isDark ? 'text-white' : 'text-black'}`}>{formatAmount(activity.amount)}</div>
                          )}
                        </div>
                      </div>
                    </div>
