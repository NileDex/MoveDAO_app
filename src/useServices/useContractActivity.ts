...
export class ContractActivityService {
  ...
  static async getContractActivities(
    ...
    try {
      ...
      // Use the correct event type path
      const eventType = `${MODULE_ADDRESS}::${this.MODULE_NAME}::ActivityEvent` as `${string}::${string}::${string}`;
      ...
      try {
        ...
        if (query.dao_address) {
          events = await aptosClient.getAccountEventsByEventType({
          ...
          events = await aptosClient.getModuleEventsByEventType({
            eventType,
            options: { limit: query.limit || 100, offset: query.offset || 0 },
          });
        }
