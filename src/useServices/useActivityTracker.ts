...
export class ActivityTracker {
    ...
    try {
      ...
      const contextFilteredActivities = activities.filter(activity => {
        ...
        if (activity.dao && activity.dao !== daoAddress && activity.dao !== 'unknown') {
          console.log(`❌ Excluding activity for different DAO: ${activity.dao}`);
          return false;
        }
      ...
      if (paginatedActivities.length === 0) {
        console.log(`⚠️ No activities found for DAO ${daoAddress}, creating sample activities for demonstration`);
  ...
  private static async fetchGlobalActivitiesPaginated(
    ...
    try {
      ...
      console.log(`📋 Found ${daos.length} DAOs to fetch activities from`);
      
      if (daos.length === 0) {
        console.log('ℹ️ No DAOs found, falling back to general transaction scan');
      ...
      const allActivitiesPromises = daos.map(async (dao) => {
        try {
          ...
          console.log(`👥 Found ${members.length} members in ${dao.name}`);
      ...
      switch (eventType) {
        case 'StakeEvent':
          return {
            ...
            type: 'stake',
            title: 'Tokens Staked',
            description: `Staked ${BalanceService.octasToMove(data.amount).toFixed(2)} MOVE`,
            amount: BalanceService.octasToMove(data.amount),
            user: sender,
        ...
        case 'UnstakeEvent':
          return {
            ...
            description: `Unstaked ${BalanceService.octasToMove(data.amount).toFixed(2)} MOVE`,
            amount: BalanceService.octasToMove(data.amount),
            user: sender,
            dao: data.dao_address || filterDaoAddress || '',
            timestamp,
        ...
        case 'TreasuryDepositEvent':
          return {
            ...
            description: `Deposited ${BalanceService.octasToMove(data.amount).toFixed(2)} MOVE to treasury`,
            amount: BalanceService.octasToMove(data.amount),
            user: sender,
            dao: data.dao_address || filterDaoAddress || '',
            timestamp,
        ...
        case 'TreasuryWithdrawalEvent':
          return {
            ...
            description: `Withdrew ${BalanceService.octasToMove(data.amount).toFixed(2)} MOVE from treasury`,
            amount: BalanceService.octasToMove(data.amount),
            user: sender,
            dao: data.dao_address || filterDaoAddress || '',
            timestamp,
        ...
        case 'RewardClaimedEvent':
          return {
            ...
            description: `Claimed ${BalanceService.octasToMove(data.amount).toFixed(2)} MOVE in rewards`,
            amount: BalanceService.octasToMove(data.amount),
            user: sender,
            dao: data.dao_address || filterDaoAddress || '',
            timestamp,
  ...
  private static async getAllRecentTransactionsPaginated(
    ...
    try {
      ...
      const response = await aptosClient.getTransactions({
        options: queryOptions
      });
  ...
  private static async getDAOMembers(daoAddress: string): Promise<string[]> {
    try {
      ...
      const totalMembersRes = await aptosClient.view({
        payload: {
