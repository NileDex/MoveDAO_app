...
export class PlatformStatsService {
  ...
  static async getPlatformStatsWithFallback(): Promise<PlatformStats> {
    try {
      ...
      try {
        registryInitialized = await aptosClient.view({
          payload: {
      ...
      try {
        totalDAOsFromRegistry = await aptosClient.view({
          payload: {
      ...
      if (registryDAOCount > 0) {
        ...
        for (const daoAddress of daoAddresses) {
          try {
            ...
            const memberCount = await aptosClient.view({
              payload: {
            ...
            try {
              const proposalCount = await aptosClient.view({
                payload: {
  ...
  /**
   * Get DAO addresses from events (same pattern as useFetchDAOs)
   */
