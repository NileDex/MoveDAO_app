...
export const useRealGrowthData = () => {
  ...
  const fetchDAOCreationEvents = useCallback(async () => {
    try {
      ...
      const daoCreationEvents = await aptosClient.getModuleEventsByEventType({
  ...
  const fetchVotingEvents = useCallback(async () => {
    try {
      ...
      const votingEvents = await aptosClient.getModuleEventsByEventType({
  ...
  const fetchTreasuryEvents = useCallback(async () => {
    try {
      ...
      const treasuryEvents = await aptosClient.getModuleEventsByEventType({
