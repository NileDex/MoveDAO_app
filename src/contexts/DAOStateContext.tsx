...
export const DAOStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchWalletBalance = useCallback(async (walletAddress: string): Promise<number> => {
    ...
    try {
      ...
      try {
        const resources: any[] = await retryWithBackoff(() => (aptosClient as any).getAccountResources?.({ accountAddress: walletAddress }));
        if (Array.isArray(resources)) {
          // Prefer AptosCoin specifically if present
          const aptosCoinStore = resources.find(r => typeof r?.type === 'string' 
            && r.type === `0x1::coin::CoinStore<0x1::aptos_coin::AptosCoin>`);
          const candidate = aptosCoinStore || resources.find(r => typeof r?.type === 'string' 
  ...
  const fetchDAOMembership = useCallback(async (
    ...
    const [minStakeRes, isMemberRes] = await Promise.all([
      retryWithBackoff(() => aptosClient.view({ 
        payload: { 
    ...
    try {
      ...
      const daoStakeRes = await retryWithBackoff(() => aptosClient.view({ 
        payload: { 
      ...
      try {
        ...
        const votingPowerRes = await retryWithBackoff(() => aptosClient.view({ 
          payload: { 
    ...
    try {
      const isDaoStakerRes = await retryWithBackoff(() => aptosClient.view({ 
        payload: { 
